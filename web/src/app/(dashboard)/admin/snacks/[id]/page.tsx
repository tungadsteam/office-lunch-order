...
export default function SnackOrdersPage() {
  ...
  const fetchData = async () => {
    try {
      const [menuRes, ordersRes]: any[] = await Promise.all([
        snacksService.getMenu(menuId),
        snacksService.getMenuOrders(menuId),
      ]);
  ...
  const handleFinalize = () => {
    confirmToast('🔒 Chốt đơn?', {
      description: 'Sẽ trừ tiền tất cả users đã đặt.',
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">
            {menu?.status === 'ordering' ? '🟢 Đang mở' : menu?.status === 'settled' ? '✅ Đã chốt' : '❌ Đã hủy'}
          </p>
        </div>
